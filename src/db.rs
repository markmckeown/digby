...
use crate::{ClearHandler, Compressor, FreeDirPage, OverflowPageHandler, StoreTupleProcessor, TreeDeleteHandler, TreeLeafPage, TupleProcessor};
...
impl Db {
    ...
    pub fn clear(&mut self) -> () {
        ...
        let tree_root_page_no = master_page.get_global_tree_root_page_no();
        let page =  self.page_cache.get_page(tree_root_page_no);   
        let new_tree_free_page_no = ClearHandler::clear_tree(page, &mut free_page_tracker, 
            &mut self.page_cache, new_version);
    ...
    pub fn clear_table_with_delete(&mut self, table_name: &Vec<u8>, delete: bool) -> () {
        ...
        let table_root_page =  self.page_cache.get_page(table_root_page);   
        let new_table_root_page_no = ClearHandler::clear_tree(table_root_page, 
            &mut free_page_tracker, &mut self.page_cache, new_version);
        ...
        if delete {
            ...
            let (new_page, _is_deleted) = TreeDeleteHandler::delete_key(table_name, table_dir_root_page, 
                &mut self.page_cache, &mut free_page_tracker, new_version);
            ...
            new_table_root_page_no.to_le_bytes().to_vec().as_ref(), &mut self.page_cache, &mut free_page_tracker, 
                new_version, &self.compressor);
            new_table_dir_root_page_no = StoreTupleProcessor::store_tuple(table_tuple, table_dir_root_page, &mut free_page_tracker, 
                &mut self.page_cache, new_version);
        }
