...
// Header 28 bytes.
// | Page No (8 bytes) | VersionHolder (8 bytes)  | LeftLeafPage (8 bytes) | Entries (u16) | FreeSpace (u16) |
//
...
impl TreeDirPage {
    ...
    fn new(block_size: usize, page_size: usize, page_number: u64, version: u64) -> Self {
        ...
        tree_page_dir
            .page
            .set_type(crate::page::PageType::TreeDirPage);
    ...
    pub fn from_page(page: Page) -> Self {
        if page.get_type() != crate::page::PageType::TreeDirPage {
            panic!(
                "Invalid page type for TreePageDir, got {:?}",
                page.get_type()
            );
        }
    ...
    pub fn get_page_to_left(&self) -> u64 {
        let index = 16;
        let slice = &self.page.get_page_bytes()[index..index + 8];
    ...
    pub fn add_entries(&mut self, entries: Vec<TreeDirEntry>) -> () {
        ...
        assert!(
            self.can_fit_entries(&entries),
            "Cannot fit entries into tree dir page."
        );
        assert!(
            entries.windows(2).all(|w| w[0].get_key() <= w[1].get_key()),
    ...
    fn get_dir_entry_index(&self, index: u16) -> TreeDirEntry {
        ...
        // Bogus - should not need to know how to interpret the tree_dir_entry structure here.
        let _page_no = tree_dir_cursor
            .read_u64::<byteorder::LittleEndian>()
            .unwrap();
        let key_len = tree_dir_cursor.read_u8().unwrap() as usize;
        let tree_dir_entry_size = key_len + 8 + 1;
        TreeDirEntry::from_bytes(
            self.page.get_page_bytes()[tree_dir_index..tree_dir_index + tree_dir_entry_size]
                .to_vec(),
        )
    }
    ...
    fn set_page_no_for_key(&mut self, key: Vec<u8>, new_page_no: u64) {
        let page_size = self.page.page_size;
        let entries = self.get_entries();
        assert!(entries != 0);
